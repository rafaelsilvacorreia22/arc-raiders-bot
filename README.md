...
- **Contagem de jogadores online** do ARC Raiders na Steam
...
- **Steam Web API** (pública, sem chave): número de jogadores online (`ISteamUserStats/GetNumberOfCurrentPlayers`) e notícias (`ISteamNews/GetNewsForApp`), App ID do ARC Raiders: `1808500`.
...
Os horários evitam cair exatamente em hora cheia (ex: `22:00`), porque o GitHub avisa que agendamentos assim são os mais sujeitos a atraso ou até serem descartados (é quando mais workflows do mundo inteiro tentam rodar ao mesmo tempo). Por isso usamos `22:05` em vez de `22:00`, por exemplo. São os mesmos horários usados no bot do WARDOGS.
...
- `scripts/post-news.js` — busca notícias oficiais do ARC Raiders, traduz pra português e posta as que ainda não foram postadas (ou a mais recente, se rodado manualmente).
- `scripts/state.json` — guarda quais notícias já foram postadas (evita repetir). Atualizado automaticamente.
- `.github/workflows/arc-raiders-player-count.yml` — agendamento da contagem de jogadores (diário + fim de semana).
- `.github/workflows/arc-raiders-news.yml` — agendamento da checagem de notícias (a cada 30 min).
...
- **Avatar do bot**: Discord → canal → Editar canal → Integrações → Webhooks → clique no avatar do "ARC Raiders Stats" → escolher imagem.
